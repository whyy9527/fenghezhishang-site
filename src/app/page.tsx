...
const VALUES = [
  {
    title: '临床为本',
    description:
      '围绕真实临床场景，强调可验证的改进与合规边界。',
  },
  {
    title: '数字化方法',
    description:
      '信息展示、数据化协作与流程在线化，提升效率与一致性。',
  },
  {
    title: '安全与合规',
    description:
      '不涉诊疗与线上交易，严格遵循信息展示规范与数据保护要求。',
  },
]
...
export default function Home() {
  return (
    <main className="bg-background text-foreground">
        ...
        <div className="mx-auto flex max-w-[1120px] flex-col gap-[24px] px-[20px] py-[72px] md:flex-row md:items-center md:justify-between md:py-[96px]">
          <div className="max-w-[720px] space-y-[20px]">
            ...
            <div className="flex flex-col gap-[12px] sm:flex-row">
              <Button className="h-[40px] px-[16px] text-[14px] leading-[22px] bg-[var(--brand-600)] hover:bg-[var(--brand-700)]" asChild>
      ...
      <section className="border-b border-[var(--border)]" aria-labelledby="values-heading">
        <div className="mx-auto max-w-[1120px] px-[20px] py-[64px]">
          ...
          <div className="grid gap-[16px] md:grid-cols-3">
            {VALUES.map((item) => (
              <Card key={item.title} className="rounded-2xl border border-[var(--border)]">
      ...
      <section className="border-b border-[var(--border)]" aria-labelledby="audience-heading">
        <div className="mx-auto flex max-w-[1120px] flex-col gap-[32px] px-[20px] py-[64px] md:flex-row">
          ...
          <ul className="grid flex-1 gap-[12px] text-[14px] leading-[22px]">
            {AUDIENCES.map((item) => (
              <li
                key={item}
                className="rounded-[16px] border border-[var(--border)] bg-[var(--surface)] px-[20px] py-[16px]"
              >
                {item}
              </li>
            ))}
      ...
      <section aria-labelledby="plan-heading">
        <div className="mx-auto flex max-w-[1120px] flex-col gap-[32px] px-[20px] py-[64px] md:flex-row">
          ...
          <div className="flex-1 space-y-[16px]">
            <div className="rounded-[16px] border border-[var(--border)] bg-[var(--surface)] px-[20px] py-[16px]">
